...
sample_paths <- list.files(file.path(stem, "outputs"), pattern = end_string)
sample_basenames <- stringr::str_remove(sample_paths, end_string)
sample_summaries <- lapply(file.path(stem, "outputs", sample_paths),
...
write.csv(table_final,
          file.path(stem, "combined_outputs" , "centrifuge_summary.csv"),
