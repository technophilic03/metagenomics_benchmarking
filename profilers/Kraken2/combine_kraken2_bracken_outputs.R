...
sample_names <- read.table(file.path(out_path, "test_lluch_samplenames.txt"),
                           sep = " ", header = FALSE) |>
  unlist() |>
  basename() |>
  stringr::str_remove_all("_taxa.txt")

all_lines <- readLines(file.path(out_path, "test_lluch.txt")) |>
...
write.csv(table_br_final,
          file.path(stem, "profilers/Bracken/combined_files" , "bracken_final_summary.csv"),
          quote = FALSE, row.names = FALSE)

write.csv(table_kr_final,
          file.path(stem, "profilers/Kraken2/combined_files" , "kraken2_final_summary.csv"),
          quote = FALSE, row.names = FALSE)


# Sanity check ----
col_sums_br <- table_br_final |>
  select(where(is.numeric)) |>
  colSums()

col_sums_kr <- table_kr_final |>
  select(where(is.numeric)) |>
  colSums()

cbind(col_sums_kr, col_sums_br) |>
  View()
